...
def mult_rev_1_arg(b_bits, b, a_bits, controlado=False):
    ...
    if controlado:
        circuito = QuantumCircuit(reg_c, reg_01, reg_a, reg_0, reg_cout)
    else:
    ...
    for i in range(b_bits):
        if s[i] == '1':
            if controlado:
                circuito.append(normal_draper_adder(a_bits + i, controlado=True), reg_c[:] + reg_01[b_bits - 1 - i:] + reg_a[:] + reg_0[:] + reg_cout[:i+1])
            else:
...
print(qc)
'''
#print(mult_rev_1_arg(3, 7, 4, controlado=True))
